...
```{r}
names <- str_extract(records, "^\\S+\\s\\S+")
hire_dates <- str_extract(
  str_extract(records, "\\|.+\\|"),
  "[[:alnum:]].+[[:alnum:]]")
...
# can
hire_dates_list <- list()
for (i in seq_along(hire_dates)) {
  hire_dates_list[[i]] <- as.Date(
    hire_dates[i],
    tryFormats = c(
...
hire_dates <- c(as.Date(NULL)) # NULL needs to be a Date o.e. it breaks
for (i in seq_along(hire_dates_list))
  hire_dates <- c(hire_dates, hire_dates_list[[i]])

rm(hire_dates_list) # don't need it anymore
```
