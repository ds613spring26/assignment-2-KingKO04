---
title: "assignment_2"
format: html
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo       = TRUE, 
                      fig.height = 3, 
                      fig.width  = 6,
                      fig.align  = "center")
ggplot2::theme_set(ggplot2::theme_bw())
```

```{r}
#| message: false
library(tidyverse)

library(lubridate)
```

# Question 1:

Write a function `pos_na()` that takes two vectors of equal length and returns the positions where **both** vectors contain `NA`.\
If the vectors do **not** have the same length, the function should return the message:

> `"The vectors are not the same length."`

Use the following pairs of vectors to test your function:

1.  `c(1, NA, 2)` and `c(NA, NA)`
2.  `c(NA, NA, 2)` and `c(NA, NA, 2)`
3.  `c(NA, 5, NA)` and `c(NA, NA, NA)`
4.  `c(NA, NA, NA, 2, NA, 4, NA)` and `c(NA, NA, 2, 4, 4, NA, NA)`
5.  `c(1, NA, NA, 2, NA, 4, NA)` and `c(NA, -5, 2, 4, 4, NA, 11)`

Here’s a polished, professional way to label those two parts of the assignment. It reads cleanly and sets expectations without adding extra wording.

------------------------------------------------------------------------

### (1i) Using an `if` statement

Write a version of `pos_na()` that checks whether the two input vectors have the same length using an `if` statement.

-   If they do, return the positions where both vectors contain `NA`.

-   If they do not, return the message:

> `"The vectors are not the same length."`

```{r}
# pos_na function
pos_na <- function(v1, v2) {
  positions <- c()
  if (length(v1) == length(v2)) {
    for (i in seq_along(v1))
      if (is.na(v1[i]) & is.na(v2[i]))
        positions <- c(positions, i)
    return(positions)
  }
  
  return("The vectors are not the same length.")
}

# tests
pos_na(c(1, NA, 2), c(NA, NA)) # The vectors are not the same length.
pos_na(c(NA, NA, 2), c(NA, NA, 2)) # 1 2
pos_na(c(NA, 5, NA), c(NA, NA, NA)) # 1 3
pos_na(c(NA, NA, NA, 2, NA, 4, NA), c(NA, NA, 2, 4, 4, NA, NA)) # 1 2 7
pos_na(c(1, NA, NA, 2, NA, 4, NA), c(NA, -5, 2, 4, 4, NA, 11)) # NULL
```


### (1ii) Without using an `if` statement

Write a second version of `pos_na()` that performs the same task **without** using an `if` statement.

-   You may use any other valid R mechanism (e.g., `stop()`, `stopifnot()`, or logical short‑circuiting) to handle the case where the vectors differ in length.

```{r}
# pos_na function
pos_na <- function(v1, v2) {
  
  i <- 1
  positions <- c()
  while (length(v1) == length(v2)) {
    while (i <= length(v1)) {
      while (is.na(v1[i]) & is.na(v2[i])) {
        positions <- c(positions, i)
        break
      }
      i <- i + 1
    }
    
    return(positions)
  }
  
  return("The vectors are not the same length.")
}

# tests
pos_na(c(1, NA, 2), c(NA, NA)) # The vectors are not the same length.
pos_na(c(NA, NA, 2), c(NA, NA, 2)) # 1 2
pos_na(c(NA, 5, NA), c(NA, NA, NA)) # 1 3
pos_na(c(NA, NA, NA, 2, NA, 4, NA), c(NA, NA, 2, 4, 4, NA, NA)) # 1 2 7
pos_na(c(1, NA, NA, 2, NA, 4, NA), c(NA, -5, 2, 4, 4, NA, 11)) # NULL
```


\`\`

# Question 2

Load the `wmata_ridership` data frame into R from <https://dcgerard.github.io/stat_412_612/data/wmata_ridership.csv>. For each month, calculate the proportion of rides made on a given day of the month. Then make box plots of ridership proportions by day of the week. But exclude any days from 2004 and 2005.

```{r}
options(scipen = 4)

suppressMessages(wmata_ridership <- read_csv(
  "https://dcgerard.github.io/stat_412_612/data/wmata_ridership.csv"
  ))

```


## Proportion of rides made on a given day of the month.
``` {r}
wmata_ridership |>
  mutate(
    Date = as.Date(Date), 
    Month = as.factor(format(Date, "%m")),
    Day = as.factor(format(Date, "%d"))
    ) |>
  group_by(Month, Day) |>
  summarize(Total = sum(Total)) |>
  group_by(Month) |>
  mutate(Prop = proportions(Total))
```


## Box plots of ridership proportions by day of the week (excluding any days from 2004 and 2005)
``` {r}
wmata_ridership |>
  filter(Date >= "2006-01-01" | Date < "2004-01-01") |>
  ggplot(aes(wday(Date, T), proportions(Total))) + 
  geom_boxplot() + 
  labs(
    x = "Day of the Week",
    y = "Proportion of Rides",
    title = "Ridership Proportions by Day of the Week"
    )
```


# Question 3:

Write R code that extracts the elements "Bears", "Dolphins", and "Bengals" from the vector `V` shown below, and display the result.

```{r}
V <- c("Bears", "Lions", "Dolphins", "Eagles", "Bengals")
```

## (3i) Subsetting by index

```{r}
V[c(1, 3, 5)]
```

## (3ii) Subsetting using a `for` loop

```{r}
for (i in seq_along(V)) if (i %in% c(1, 3, 5)) print(V[i])
```

## (3iii) Returning all values in a single vector (one line of code)

```{r}
V[c(1, 3, 5)]
```

### (a) Using positive indices

```{r}
V[c(1, 3, 5)]
```

### (b) Using negative indices

```{r}
V[c(-0, -2, -4)]
```

### (c) Using logical indexing

```{r}
V[c(T, F, T, F,T)]
```

### (d) Attempting to subset by names

```{r}
V["Bears"]
V["Dolphins"]
V["Bengals"]
```

# Question 4:

You are given a character vector containing messy employee records. Each entry includes:

-   an employee name (string),
-   a hire date in inconsistent formats,
-   and a department code that should be treated as a factor.

```{r}
records <- c(
  "Steve McQueen | 2020-01-15 | HR",
  "B. Smith | 15/02/2021 | FIN",
  "Carlos M | March 3, 2019 | IT",
  "D. Lee | 2018/07/30 | HR",
  "Alain Delon | 04-12-2020 | MKT"
)
records
```

1.  **Parse each record** into three separate variables:

    -   `name`\
    -   `hire_date`\
    -   `dept`

```{r}
name <- str_extract(records, "^\\S+\\s\\S+")
hire_date <- str_extract(
  str_extract(records, "\\|.+\\|"),
  "[[:alnum:]].+[[:alnum:]]")
dept <- str_extract(records, "[A-Z]{2,}")
```

2.  **Convert `hire_date` into a proper `Date` object**, correctly handling all of the different date formats.

```{r}
# dates need to be changed differently. vectors cannot handle that but lists
# can
hire_date_list <- list()
for (i in seq_along(hire_date)) {
  hire_date_list[[i]] <- as.Date(
    hire_date[i],
    tryFormats = c(
      "%m-%d-%Y", # has to be first b.c. months are smaller #s than years
      "%F", 
      "%d/%m/%Y",
      "%B %d, %Y",
      "%Y/%m/%d"
      )
    )
}
hire_date <- c(as.Date(NULL)) # NULL needs to be a Date o.e. it breaks
for (i in seq_along(hire_date_list))
  hire_date <- c(hire_date, hire_date_list[[i]])

rm(hire_date_list) # don't need it anymore
```

3.  **Convert `dept` into an ordered factor** with the levels:\
    `HR`, `FIN`, `IT`, `MKT`.

```{r}
dept <- factor(dept, c("HR", "FIN", "IT", "MKT"))
```

4.  **Construct a clean data frame** containing the parsed and converted variables.

```{r}
records_df <- data.frame(name, hire_date, dept)
```

5.  **Add a new variable** called `years_worked` that reports the number of full years each employee has worked as of today.

```{r}
records_df <- mutate(
  records_df, 
  years_worked = as.integer((today() - as.Date(hire_date)) / 365)
    )
```

6.  **Return the final cleaned data frame.**

```{r}
records_df
```

